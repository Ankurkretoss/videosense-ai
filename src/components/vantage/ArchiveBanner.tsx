...
export function ArchiveBanner({ archive }: { archive: AnalysisArchiveState }) {
  ...
  if (archive.status === "saving") {
    return (
      <div className="flex items-center gap-2 rounded-lg border border-white/[0.11] bg-panel p-3 text-sm text-mute">
        <Loader2 className="h-4 w-4 animate-spin text-brand-soft" />
        Saving this match to your library…
      </div>
    );
  }
  ...
  if (archive.status === "saved") {
    return (
      <div className="flex flex-wrap items-center gap-2 rounded-lg border border-emerald-500/30 bg-emerald-500/5 p-3 text-sm text-emerald-200/90">
        ...
        Saved to My matches — every detail of this match is stored.
        <Link href="/dashboard/matches" className="ml-auto">
          <Button size="sm" variant="outline" className="border-white/[0.11] bg-panel text-white">
  ...
  return (
    <div className="flex flex-wrap items-center gap-2 rounded-lg border border-red-500/30 bg-red-500/5 p-3 text-sm text-red-300">
      ...
      <Button
        size="sm"
        variant="outline"
        className="border-white/[0.11] bg-panel text-white"
